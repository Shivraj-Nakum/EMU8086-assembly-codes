;Write an assembly language program to add two 4-digit hexadecimal 
;numbers. Read the data through keyboard and display result on screen using 
;procedure near   
...
MOV BX,0100H ; Load BX with 1000H (place value of 2
nd hex digit)
MUL BX ; Multiply AX with BX (weightage value of 2
nd digit in AX)
ADD TEMP, AX ; Add TEMP with AX (add weightage of 1st and 2
nd digit)
MOV AH,01H ; Use DOS 01H for 3
rd digit entry through keyboard
INT 21H
CMP AL,39H ; Compare ASCII hex value of key pressed (AL) with 39H
...
MOV BX,0010H ; Load BX with 0010H (place value of 3
rd hex digit)
MUL BX ; Multiply AX with BX (weightage value of 3
rd digit in AX)
ADD TEMP, AX ; Add TEMP with AX (weightage of 3
rd digit + previous)
MOV AH,01H ; Use DOS 01H for 4
th digit entry through keyboard
INT 21H
CMP AL,39H ; Compare ASCII hex value of key pressed (AL) with 39H
...
MOV AH, 02H ; Use of DOS 02H for 1
st hex digit ASCII hex value
...
MOV CX, 1000H ; Load CX with 1000H as divisor (place value of 2
nd digit)
...
JMP K2 ; Jump to label K2 for 2
nd hex digit display on screen
K1: ADD AL, 37H ; If AL > 09H, add 37H to AL for 2nd digit ASCII value
K2: MOV DL, AL ; Load DL with AL (2
nd digit ASCII hex)
MOV AH, 02H ; Use of DOS 02H for 2
nd hex digit ASCII hex value
INT 21H ; display on screen
MOV AX, RES ; Load AX with RES value (lower word of dividend)
MOV DX, 0000H ; Load DX with 0000H value ((upper word of dividend)
MOV CX, 0100H ; Load CX with 0100H as divisor (place value of 3
rd digit)
DIV CX ; Divide DX:AX by CX
MOV RES, DX ; Store DX into RES for next division (Remainder)
CMP AL, 09H ; Compare AL with 09H
JA K3 ; If AL > 09H, go to label K3 (for A to F hex digit)
ADD AL, 30H ; If AL = 09H, add 30H to AL for 3
rd digit ASCII value
JMP K4 ; Jump to label K4 for 3
rd hex digit display on screen
K3: ADD AL, 37H ; If AL > 09H, add 37H to AL for 3
rd digit ASCII value
K4: MOV DL, AL ; Load DL with AL (3
rd digit ASCII hex)
MOV AH, 02H ; Use of DOS 02H for 3
rd hex digit ASCII hex value
INT 21H ; display on screen
MOV AX, RES ; Load AX with RES value (lower word of dividend)
MOV DX,0000H ; Load DX with 0000H value ((upper word of dividend)
MOV CX,0010H ; Load CX with 0100H as divisor (place value of 4
th digit)
DIV CX ; Divide DX:AX by CX
...
K6: MOV DL, AL ; Load DL with AL (4
th digit ASCII hex)
MOV AH,02H ; Use of DOS 02H for 4
th hex digit ASCII hex value
INT 21H ; display on screen 
...
K8: MOV AH, 02H ; Use of DOS 02H for 5
th hex digit ASCII hex value
INT 21H ; display on screen
